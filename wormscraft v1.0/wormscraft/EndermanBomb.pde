class EndermanBomb extends Bullet
{  
  EndermanBomb(float x, float y, float _angle, float _power)
  {
  ...
  {    
    ...
    {
      BasicAI test = new EnderMan((int)this.physics.position.x + 1, (int)this.physics.position.y);
      game.addNPC(test);
